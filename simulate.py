...
# Windows consoles default to cp1252, which cannot encode sigma/box-drawing
# characters used in the printouts.  Force UTF-8 where supported.
import sys
...
def run(
    ...
    for k in range(n_steps):
        ...
        try:
            ...
            # ── Divergence guard ─────────────────────────────────────────
            # Abort cleanly instead of integrating to float overflow / NaN.
            if (not np.all(np.isfinite(x))) or np.max(np.abs(x)) > 1e4:
                print(f"\n  *** DIVERGENCE at t={t:.2f}s — state left sane "
                      f"bounds (max |x| = {np.max(np.abs(x)):.3g}). "
                      f"Aborting simulation. ***")
                n_steps = k + 1
                break
    ...
    if verbose:
        ...
        if att_ctrl.solve_times:
            st = np.array(att_ctrl.solve_times) * 1e3   # → ms
            print(f"  QP solve time     : mean {st.mean():.2f} ms | "
                  f"max {st.max():.2f} ms | n={len(st)}")
