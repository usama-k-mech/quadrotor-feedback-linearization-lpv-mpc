...
class PositionController:
    ...
    def __init__(
        ...
        # Physical thrust ceiling: 90% of max total rotor thrust.  The exact
        # inversion computes U1 = m·‖F_W‖, which grows without bound with
        # position error; clamping prevents commanding thrust the rotors
        # cannot deliver (divergence guard, see 2026 gravity-sign postmortem).
        self.U1_max = 0.9 * 4.0 * params.c_T * params.omega_max**2
    ...
    def compute(
        ...
        U1 = float(min(m * Fw_norm, self.U1_max))   # clamp to physical limit
...
class LPVMPCController:
    ...
    def __init__(
        ...
        self.solve_times = []               # per-step QP wall time [s]
    ...
    def compute(
        ...
        if _QUADPROG_AVAILABLE:
            ...
            dU_opt = res.x
        self.solve_times.append(perf_counter() - _t_qp0)
